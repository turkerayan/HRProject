...
const AdvanceDeletedModal = ({modalOption,setModalOptions,title}) => {

    const {setAdvances} = useContext(ApiContext);
    const {notify} = useContext(DataContext)

    const deletePermission= async () =>{
        const response = await AdvanceService.DeleteAdvancePayment(modalOption.id)
...
export default AdvanceDeletedModal